...
# for (i in 1:80){
  ...
  ground_truth_all <- c(ground_truth_all, ground_truth_vec)
  ground_truth_set <- unique(ground_truth_vec)
  ...
  tesseract_vec <- str_split(clean_tesseract_txt," ")[[1]] # tokens
  nonerror_train_list[[i]] <- tesseract_vec[tesseract_vec %in% ground_truth_set]
  error_train_list[[i]] <-tesseract_vec[!tesseract_vec %in% ground_truth_set]
}
...
# for (i in 1:20){
  ...
  current_tesseract_txt <- readLines(paste("../data/tesseract/",current_file_name,".txt",sep=""), warn=FALSE)
  clean_tesseract_txt <- paste(current_tesseract_txt, collapse = " ")
  tesseract_vec_list[[i]] <- str_split(clean_tesseract_txt," ")[[1]] 
  test_labels_list[[i]] <- as.factor(ifelse(tesseract_vec_list[[i]] %in% ground_truth_set, 1, 0))
  # tesseract_if_clean <- unlist(lapply(tesseract_vec,ifCleanToken))
  # nonerror_test_list[[i]] <- tesseract_vec[tesseract_vec %in% ground_truth_set]
  # error_test_list[[i]] <-tesseract_vec[!tesseract_vec %in% ground_truth_set]
}
