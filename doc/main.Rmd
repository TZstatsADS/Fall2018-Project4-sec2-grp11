...
# error_train_list <- rep(list(NULL),80)
# nonerror_train_list <- rep(list(NULL), 80)
line_diff <- c()
equal_line_ind <-c()
ground_truth_all <- c()

train_file_id <-c(9:10, 37:38, 41, 67:68,98:100)
error_train_list <- rep(list(NULL),10)
nonerror_train_list <- rep(list(NULL), 10)

# token_match <- function(token, truth_line){
#   return(ifelse(token %in% truth_line, TRUE, FALSE))
# }
# line_match <- function(tesseract_line, truth_line){
#   line_clean_ind <- unlist(lapply(tesseract_line, token_match, truth_line))
#   return(line_clean_ind)
# }
...
for (i in 1:10){
# for (i in 1:80){
  current_file_name <- sub(".txt","",file_name_vec[train_file_id[i]])

  ## read the ground truth text
  current_ground_truth_txt <- readLines(paste("../data/ground_truth/",current_file_name,".txt",sep=""), warn=FALSE)
  current_ground_truth_list <- as.list(current_ground_truth_txt)
  current_ground_truth_list_vec <- lapply(current_ground_truth_list, string_split_1st)
  current_ground_truth_vec <- unlist(current_ground_truth_list_vec)
  ground_truth_all <- c(ground_truth_all, current_ground_truth_vec)

  ## read the tesseract text
  current_tesseract_txt <- readLines(paste("../data/tesseract/",current_file_name,".txt",sep=""), warn=FALSE)
  current_tesseract_list <- as.list(current_tesseract_txt)
  current_tesseract_list_vec <- lapply(current_tesseract_list, string_split_1st)
  current_tesseract_vec <- unlist(current_tesseract_list_vec)
  
  #compare tesseract text and ground truth
  match_result <- doc_match(current_tesseract_list_vec, current_ground_truth_list_vec)
  current_clean_ind <- match_result[[1]]
  equal_line_ind <- c(equal_line_ind, match_result[[2]])
  line_diff <- c(line_diff, match_result[[3]])

  nonerror_train_list[[i]] <- current_tesseract_vec[current_clean_ind]
  error_train_list[[i]] <-current_tesseract_vec[!current_clean_ind]
}

# bigrams_all <- unlist(lapply(ground_truth_all, bigram_from_token))
eng_bigrams <- unlist(lapply(english.words, bigram_from_token))
ground_truth_all_set <- unique(ground_truth_all)

nonerror_train <- unique(unlist(nonerror_train_list))
error_train <- unique(unlist(error_train_list))
train_tokens <- c(nonerror_train,error_train)
train_labels <- as.factor(c(rep(1, length(nonerror_train)),rep(0, length(error_train))))
# source('../lib/features.R')
system.time(train_input <-matrix(unlist(lapply(train_tokens,extract_feature, eng_bigrams, ground_truth_all_set)), byrow = TRUE, nrow = length(train_tokens)))
...
cv_result<-matrix(NA, nrow = 3, ncol = 3)
# Cs <- c(0.01, 0.1, 1)
# sigmas <- c(0.01, 0.1, 1)
# cv_time <- matrix(NA, nrow = length(Cs), ncol = length(sigmas))
gammas <- c(1, 10, 25)
lambdas <- c(0.00005, 0.0002,0.001)
for(i in 1:length(gammas)){
  for( j in 1:length(lambdas)){
    pars = list("gamma"=gammas[i],"lambda"=lambdas[j])
    system.time(cv_result[i,j] <- cv.function(train_input,train_labels,pars,K))
  }
}
cv_result
best_ind <- which(cv_result == max(cv_result), arr.ind = TRUE)
best_gamma <- gammas[best_ind[1]]
...
best_gamma <- 10
best_lambda <- 0.0002
system.time(fit_svm <- mcSVM(x = train_input, y = train_labels, 
                             gamma = best_gamma, lambda = best_lambda, KERNEL = "GAUSS_RBF"))
```
...
# tesseract_vec_list <- rep(list(NULL), 20)
# test_labels_list <- rep(list(NULL), 20)
test_equal_line_ind <- c()
test_line_diff <- c()
test_file_id <-c(1, 11, 39, 42, 69)
tesseract_vec_list <- rep(list(NULL), 5)
test_labels_list <- rep(list(NULL), 5)

for (i in 1:5){
# for (i in 1:20){
  current_file_name <- sub(".txt","",file_name_vec[test_file_id[i]])
  ...
  current_tesseract_list_vec <- lapply(current_tesseract_list, string_split_1st)
  current_tesseract_vec <- unlist(current_tesseract_list_vec)
  tesseract_vec_list[[i]] <- current_tesseract_vec
  
  #compare tesseract text and ground truth
  match_result <- doc_match(current_tesseract_list_vec, current_ground_truth_list_vec)
  current_clean_ind <- match_result[[1]]
  test_equal_line_ind <- c(test_equal_line_ind, match_result[[2]])
  test_line_diff <- c(test_line_diff, match_result[[3]])

  test_labels_list[[i]] <- as.factor(ifelse(current_clean_ind, 1, 0))
}
test_tokens <- unlist(tesseract_vec_list)
test_labels <- unlist(test_labels_list)
system.time(test_input <- matrix(unlist(lapply(test_tokens,extract_feature,eng_bigrams, ground_truth_all_set)), byrow = TRUE, nrow = length(test_tokens)))
# test_input_na <- test_input
# test_input_na[is.infinite(test_input_na)]<-NA
# test_input_na[is.nan(test_input_na)]<-NA
system.time(preds <- predict(fit_svm, test_input))
table(preds, test_labels)
# system.time(preds <- predict(model, test_input_na))


# system.time(preds_auto <- predict(fit_svm_auto, test_input_na))
# table(preds_auto, test_labels)
# system.time(predsmc <- predict(fit_mcsvm, test_input_na))
# table(predsmc, test_labels)
# 
# system.time(predsmc2 <- predict(fit_mcsvm2, test_input_na))
# table(predsmc2, test_labels)
```

detection evaluation
The problem with the algorithm, even embedded in the evaluation is that it is token-wise. A correct token in itself does not necessarily mean that it is right in that position. e.g. The OCR read "staff" as "5" in the first file, but "5" may exists in the whole corpus for the ground truth. In this way, a "5" at any position will be treated as correct. But obviously we cannot assign some of "5"s to the correct class, some to "error", because the svm classification is token-by-token.

```{r}
#false negative for errors causes more severe consequence than false positive, as there is a procedure for correction after the detection which can somehow filter out the false postive, while false negative are errors that are totally ignored

#correctly classified nonerror
#mostly stopwords
sample(test_tokens[preds== "1" & test_labels == "1"], 100)
correct_nonerror_features <- round(colMeans(test_input[which(preds =="1" & test_labels == "1"),]),2) 
correct_nonerror_features
```

correctly classified errors
```{r}
sample(test_tokens[preds== "0" & test_labels == "0"], 100)
correct_error_features <- round(colMeans(test_input[which(preds =="0" & test_labels == "0"),]),2)
correct_error_features
```
classified nonerror as error
potential cause 1: too many numbers rejected
suggestion: put more weight on the feature indicating numeric strings for classification
potential cause 2: punctuations following a correct string
suggestion: try modifying features that take into account such cases
potential cause 3: the ground truth itself contains formatting error, e.g. expected separation in a single word
potential cause 4: english names not included in english words
```{r}
sample(test_tokens[preds== "0" & test_labels == "1"],100)
nonerror_as_error_features <- round(colMeans(test_input[which(preds =="0" & test_labels == "1"),]),2)
nonerror_as_error_features
```

classified error as nonerror
cause: procedure of real label assignment?
remedy:??? regression defect?
```{r}
sample(test_tokens[preds== "1" & test_labels == "0"],100)
error_as_nonerror_features <- round(colMeans(test_input[which(preds =="1" & test_labels == "0"),]),2)
error_as_nonerror_features
```

examine vocabulary-based features effect on svm (bigram & levenshtein distance)
suggestion: add more vocabulary-based features and modify the self-based features to fit the classificaiton of error and nonerror instead of garbage and nongarbage
```{r}
#in fact need another CV here
#using features excluding 2
train_input_self <- train_input[,-c(19,22)]
test_input_self <- test_input[,-c(19,22)]
system.time(fit_svm_self <- mcSVM(x = train_input_self, y = train_labels, 
                                  C = best_C, sigma = best_sigma, KERNEL = "GAUSS_RBF"))
system.time(preds_self <- predict(fit_svm_self, test_input_self))
table(preds_self, test_labels)

#using only non-vocabulary-based features defined in the paper for garbage detection, we get more false positives for errors. The overall performance is not obviously better than a random classifier. 
```
```{r}
#in fact need another CV here
train_input_voc <- train_input[,c(19,22)]
test_input_voc <- test_input[,c(19,22)]
system.time(fit_svm_voc <- mcSVM(x = train_input_voc, y = train_labels,
                                 C = best_C, sigma = best_sigma, KERNEL = "GAUSS_RBF"))
system.time(preds_voc <- predict(fit_svm_voc, test_input_voc))
table(preds_voc, test_labels)
#using only vocabulary-based features is slightly better than non-vocabulary-based features, but it is still much worse than using all the features. But putting together the false negatives for errors are worse than using only vocabulary-based features, and the true positives for errors are slightly worse. Considering the later procedure for correction, maybe using only vocabulary-based features are better for the overall OCR post-processing performance. But it also depends on the correction methods - if it is able to leave the tokens that don't need to be corrected as they are and the time it takes for correction.
```
