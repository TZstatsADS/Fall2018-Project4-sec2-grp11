...
train_file_id <-c(1:8, 11:32, 39:40, 42:63, 69:94)
...
# source('../lib/features.R')
train_input <-matrix(unlist(lapply(train_tokens,extract_feature)), byrow = TRUE, nrow = length(train_tokens))
...
system.time(fit_svm[[1]] <- ksvm(x = train_input_na, y = train_labels, type = "nu-svc", nu = 0.5, cross=3))
system.time(fit_svm[[2]] <- ksvm(x = train_input_na, y = train_labels, type = "nu-svc", nu = 0.6, cross=3))
system.time(fit_svm[[3]] <- ksvm(x = train_input_na, y = train_labels, type = "nu-svc", nu = 0.7, cross=3))
```

test data
```{r}
test_file_id <-c(9:10, 33:38, 41, 64:68, 95:100)
# error_test_list <- rep(list(NULL), 20)
# nonerror_test_list <- rep(list(NULL), 20)
