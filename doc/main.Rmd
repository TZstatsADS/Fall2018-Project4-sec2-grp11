...
#   current_tesseract_txt <- readLines(paste("../data/tesseract/",current_file_name,".txt",sep=""), warn=FALSE)
#   current_tesseract_list_vec <- lapply(current_tesseract_txt, stringsplit_1st)
#   current_correction_list <- match_correct(current_tesseract_list_vec, current_ground_truth_list_vec)
#   corrections_list_vec[[i]] <- current_correction_list
# }
# 
# #try matching error_list_vec with corrections_list_vec? or just use those corrections?
```

```{r}
#Pure correciton using real errors
test_corrected_list_vec <- list()
test_error_list_vec <- error_list_vec[test_file_id]
test_corrections_list_vec <- corrections_list_vec[test_file_id]
for (i in 1:20){
  test_corrected_list_vec[[i]] <- lapply(test_error_list_vec[[i]], do_correction_on_vec)
}
#evaluation using test_corrections_list_vec & test_corrected_list_vec
#word level
num_corrected <- rep(0, 20)
for(i in 1:20){
  num_corrected[i] <- sum(unlist(doc_match(test_corrected_list_vec[[i]], test_corrections_list_vec[[i]])))
}
total_corrected <- sum(num_corrected)


# # #correction after detection from svm
# test_tokens_list_vec <- tesseract_list_vec[test_file_id]
# test_tokens_per_line_list <- tokens_per_line_list[test_file_id]
# preds_list_vec <- list()
# for(i in 1:20){
#   doc_line_lens <- test_tokens_per_line_list[[i]] # a vector: e.g. 5 7 9 ...
#   doc_lines <- length(doc_line_lens) #number of lines in the current doc
#   doc_preds <- preds_list[[i]]# the svm preds vector for the current doc
#   preds_list_vec[[i]] <- list()
#   for(j in 1:doc_lines){
#     current_line_preds <- doc_preds[1:doc_line_lens[j]] # get the preds for the current line
#     doc_preds <- doc_preds[(doc_line_lens[j]+1):length(doc_preds)] # delete the preds for the current line
#     preds_list_vec[[i]] <- list.append(preds_list_vec[[i]], current_line_preds)
#   }
# }
# 
# predicted_error_list_vec <- list()
# for (i in 1:20){
#   current_file_tokens_list <- test_tokens_list_vec[[i]]
#   current_file_preds_list <- preds_list_vec[[i]]
#   l = length(current_file_tokens_list)
#   predicted_error_list_vec[[i]] <- list()
#   for(j in 1:l){
#     predicted_error_list_vec[[i]] <- list.append(predicted_error_list_vec[[i]], current_file_tokens_list[[j]][!current_file_preds_list[[j]]])
#   }
# }
# svm_corrected_list_vec <- list()
# for (i in 1:20){
#   svm_corrected_list_vec[[i]] <- lapply(predicted_error_list_vec[[i]], do_correction_on_vec)
# }
# 
...
# }
# 
# #evaluation using test_corrections_list_vec & svm_corrected_list_vec
# #???
